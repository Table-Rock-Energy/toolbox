...
- [x] **DB-01**: PostgreSQL is the sole database -- no Firestore code in any request path
...
- [x] **DB-04**: One-time migration script exports all Firestore collections and imports into PostgreSQL (service account JSON as CLI arg)
- [x] **DB-05**: Every firestore_service.py function has a working PostgreSQL equivalent in db_service.py
- [x] **DB-06**: firestore_service.py deleted and all Firestore imports/dependencies removed from codebase
...
- [x] **AI-03**: Gemini provider and google-genai dependency removed entirely -- LM Studio is the only AI backend
