...
const verificationBadge = (status: string) => {
  switch (status) {
    case 'user_verified':
    case 'user_corrected':
      return <span className="inline-flex items-center gap-1 text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full"><CheckCircle className="w-3 h-3" /> Verified</span>
    case 'high_confidence':
      return <span className="inline-flex items-center gap-1 text-xs bg-tre-teal/10 text-tre-teal px-2 py-0.5 rounded-full"><CheckCircle className="w-3 h-3" /> High Confidence</span>
    case 'disputed':
      return <span className="inline-flex items-center gap-1 text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full"><AlertCircle className="w-3 h-3" /> Disputed</span>
    default:
      return <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">Inferred</span>
  }
}
...
export default function MineralRights() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-oswald font-semibold text-gray-900">
          Database
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-oswald font-semibold text-gray-900">
                {status?.total_entities ?? '—'}
              </p>
              <p className="text-xs text-gray-500 uppercase tracking-wider">Entities</p>
            </div>
          </div>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 uppercase tracking-wider">Ownership Records</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-white border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 font-oswald focus:outline-none focus:border-tre-teal focus:ring-1 focus:ring-tre-teal transition-colors"
            />
          </div>
          <button
            onClick={handleSearch}
            disabled={searchLoading}
            className="px-6 py-3 bg-tre-teal text-white rounded-lg font-oswald hover:bg-tre-teal/90 transition-colors disabled:opacity-50"
          >
              ...
              {searchResults.map((result) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                    selectedEntity?.id === result.entity.id
                      ? 'bg-tre-teal/10 border border-tre-teal/40'
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-gray-900 font-oswald truncate">
        ...
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          {loading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-0 divide-y lg:divide-y-0 lg:divide-x divide-gray-200">
                {/* Left Column: Names, Addresses, Sources */}
                <div className="p-6 space-y-6">
                  {/* Known Names */}
                  {selectedEntity.names.length > 0 && (
                    <div>
                      <h3 className="text-sm font-oswald uppercase tracking-wider text-gray-700 mb-2">
                        Known Names
                      </h3>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div>
                    ...
                    {relationships.length > 0 ? (
                      ...
                      <p className="text-sm text-gray-400">No relationships found yet</p>
                    )}
                  </div>
                    ...
                    {ownershipRecords.length > 0 ? (
                      <div className="space-y-2">
                        {ownershipRecords.map((rec) => (
                          ...
                          >
                            <p className="text-sm text-gray-900 font-oswald">
                      ...
                      <p className="text-sm text-gray-400">No ownership records yet</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              {selectedEntity.notes && (
                <div className="p-6 border-t border-gray-200">
                  ...
                  <p className="text-sm text-gray-600">{selectedEntity.notes}</p>
                </div>
              )}
            </>
          )}
      ...
      {!selectedEntity && searchResults.length === 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-12 text-center">
          ...
          <h3 className="text-lg font-oswald text-gray-500 mb-2">
