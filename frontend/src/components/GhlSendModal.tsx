...
function mapRowsToContacts(rows: Record<string, string>[]): BulkContactData[] {
  return rows.map(row => ({
    ...
    phone_2: str(row['Phone 2']),
    phone_3: str(row['Phone 3']),
    phone_4: str(row['Phone 4']),
    phone_5: str(row['Phone 5']),
    ...
    bankruptcy: str(row['Bankruptcy']),
    deceased: str(row['Deceased']),
    lien: str(row['Lien']),
    campaign_system_id: str(row['Campaign System ID']),
...
export default function GhlSendModal({
  ...
  const [manualSms, setManualSms] = useState(true)
